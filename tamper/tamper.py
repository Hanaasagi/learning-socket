...
def send_packet(protocol=None, src_ip=None, src_port=None, flags=None, dst_ip=None, dst_port=None, iface=None):
    if protocol == 'tcp':
        packet = IP(src=src_ip, dst=dst_ip,) / \
            TCP(flags=flags, sport=src_port, dport=dst_port)
    elif protocol == 'udp':
        if flags:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iface', action='store',
                        dest='iface', default='eth0')
    parser.add_argument('--protocol', action='store',
                        dest='protocol', default='tcp')
    parser.add_argument('--src-ip', action='store',
                        dest='src_ip', default='1.1.1.1')
    parser.add_argument('--src-port', action='store',
                        dest='src_port', default=randint(0, 65535))
    parser.add_argument('--dst-ip', action='store',
                        dest='dst_ip', default='192.168.1.200')
